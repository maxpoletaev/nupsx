...
pub fn decodeBlock(block: *const [16]u8, prev_sample: *[2]i32, out: *[28]i16) void {
    ...
    for (0..28) |n| {
        ...
        sample += @divTrunc(prev_sample[0] * filter_pos + prev_sample[1] * filter_neg + 32, 64);

        out[n] = @intCast(std.math.clamp(sample, -0x8000, 0x7fff));

        prev_sample[1] = prev_sample[0];
        prev_sample[0] = sample;
    }
}
